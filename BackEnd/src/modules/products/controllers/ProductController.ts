...
class ProductController {
    ...
    public async create(request: Request, response: Response ): Promise<Response>{
        ...
        let {name, category, quantity, price, localization} = request.body
        // vamos criar objeto da classe CreateProductService
        const object = new CreateProductService()
        const newProduct = await object.execute({name, category, quantity, price, localization})
    ...
    public async update(request: Request, response: Response): Promise<Response> {
        ...
        const {id} = request.params
        const {name, category, quantity, price, localization} = request.body
        const updateProductService = new UpdateProductService()
        const productUpdated = await 
            updateProductService.execute({id, name, category, quantity, price, localization})
